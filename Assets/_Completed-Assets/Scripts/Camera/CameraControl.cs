...
namespace Complete
{
    public class CameraControl : MonoBehaviour
    {
        ...
        public void ShowAllMap(GameObject map)
        {
            // Calculate the bounds of the map
            Bounds mapBounds = new Bounds(map.transform.position, Vector3.zero);
            for (int i = 1; i < map.transform.childCount; i++)
            {
                mapBounds.Encapsulate(map.transform.GetChild(i).position);
            }

            // Set the camera position to view the entire map
